...
\newcommand{\chisq}{\chi^2_{\text{obs}}}
...
\section{Inference about variances}
\subsection*{One population}
Suppose we have \(n\) observations that are i.i.d normal with \(X_1,X_2,...,X_n\sim N(\mu,\sigma^2)\), then:
\[\frac{\sum_{i=1}^n(X_i-\bar{X})^2}{\sigma^2}=\frac{(n-1)s^2}{\sigma^2}\sim \chi^2_{n-1}\]
Which is a chi-squared distribution with \(n-1\) degrees of freedom. We use this distribution for hypothesis testing for. The chi-squared distribution is actually an example of a specific gamma distribution:
\[\chi^2_{n-1}\sim \Gamma\left(\frac{n-1}{2},\frac{1}{2}\right)\]
\[E[\chi^2_{\nu}]=\nu\]
\[\Var{\chi_\nu^2}=2\nu\]
\subsection*{Confidence intervals}
For one population (assuming i.i.d normal observations) a confidence interval for \(\sigma^2\) at an \(\alpha\) significance level can be constructed with:
\[\left(\frac{(n-1)s^2}{\chi^2_{n-1,1-\alpha/2}},\frac{(n-1)s^2}{\chi^2_{n-1,\alpha/2}}\right)\]
\subsection*{Hypothesis testing}
For a hypothesis test:
\[H_0:\sigma^2= \sigma^2_0\]
\[H_1:\sigma^2\ne\sigma^2_0\]
Under \(H_0\) we have a test statistic:
\[\frac{(n-1)s^2}{\sigma^2_0}\sim \chi^2_{n-1}\]
We can use this test statistic to compare against the critical region and reject \(H_0\) if:
\[\begin{cases}
    \frac{(n-1)s^2}{\sigma^2_0}>\chi^2_{n-1,1-\alpha/2} \\
    \frac{(n-1)s^2}{\sigma^2_0}<\chi^2_{n-1,\alpha/2}
\end{cases}\]
\subsection*{Ratio of two population variances}
Suppose we have two independent populations with random samples:
\[X_1,X_2,...,X_{n_X}\sim N(\mu_X,\sigma^2_X)\]
\[Y_1,Y_2,...,Y_{n_Y}\sim N(\mu_Y,\sigma^2_Y)\]
Then we want to test if \(\frac{\sigma^2_X}{\sigma^2_Y}=1\), that is if the variances are equal. We already know:
\[\frac{(n_X-1)s^2}{\sigma^2_X}\sim\chi^2_{n_X-1},\qquad \frac{(n_Y-1)s^2}{\sigma^2_Y}\sim\chi^2_{n_Y-1}\]
A useful result to know is that:
\[F=\frac{s^2_X/\sigma^2_X}{s^2_Y/\sigma^2_Y}\sim F_{n_X-1,n_Y-1}\]
Where \(F_{\nu_1,\nu_2}\) is an \(F\) distribution with \(\nu_1,\nu_2\) degrees of freedom. We use this for distribution for hypothesis testing, particularly when we have the situation:
\[H_0:\sigma^2_X=\sigma^2_Y\]
\[H_1:\sigma_X^2\ne\sigma^2_Y\]
Under \(H_0\) we have,
\[F=\frac{s^2_X}{s^2_Y}\sim F_{n_X-1,n_Y-1}\]
We can compare this statistic to our critical region at an \(\alpha\) significance level and reject \(H_0\) if:
\[\begin{cases}
    F>F_{n_X-1,n_y-1,1-\alpha/2} \\
    F<F_{n_X-1,n_Y-1,\alpha/2}
\end{cases}\]
Note that the \(F\) distribution table only gives us the upper critical value, however we can convert between the two by the formula:
\[F_{\nu_1,\nu_2,\alpha}=\frac{1}{F_{\nu_2,\nu_1,1-\alpha}}\]
\section{Inferences about proportions}
\subsection*{Normal approximation}
If we have a large sample size \(n\) then we can approximate a normal distribution with a binomial one, which allows us to perform lots more analysis on it:
\[X\sim \text{Bin}(n,\pi)\approx N(n\pi,n\pi(1-\pi))\]
\subsection*{Single population}
We can use the normal approximation to construct a hypothesis test for a sample proportion being equal to the true population proportion:
\[H_0:\pi=\pi_0\]
\[H_1:\pi\ne\pi_0\]
Under \(H_0\) at a significance level of \(\alpha\) we have the test statistic:
\[|z|=\left|\frac{X-n\pi_0}{\sqrt{n\pi_0(1-\pi_0)}}\right|\]
Which we can compare to the critical region, \(z_{1-\alpha/2}\). Equivalently we can write our test in terms of the sample proportion \(p=\frac{X}{n}\), then:
\[|z|=\left|\frac{p-\pi_0}{\sqrt{\frac{\pi_0(1-\pi_0)}{n}}}\right|\]
Which we compare with the same critical region. It can also be worthwhile to construct a confidence interval for a single population proportion - this happens a lot when we look at opinion polls in politics.
\[p\pm z_{1-\alpha/2}\sqrt{\frac{p(1-p)}{n}}\]
For smaller \(n\) we can also consider:
\[\frac{|X-n\pi_0|-\frac{1}{2}}{\sqrt{n\pi_0(1-\pi_0)}}\sim N(0,1)\]
Which applies a "continuity correction" which can help account for the fact we are changing continuity in our approximation. For large \(n\) this correction factor does not matter because the central limit theorem kicks in.
\subsection*{Comparing two population proportions}
If we assume both samples are independent and identically distributed then we have:
\[p_1\sim N\left(\pi_1,\frac{\pi_1(1-\pi_1)}{n_1}\right),\qquad p_2\sim N\left(\pi_2,\frac{\pi_2(1-\pi_2)}{n_2}\right)\]
But since they are independent we can also write a new random variable:
\[p_1-p_2\sim N\left(\pi_1-\pi_2,\frac{\pi_1(1-\pi_1)}{n_1}+\frac{\pi_2(1-\pi_2)}{n_2}\right)\]
We want to test under the assumption \(H_0:\pi_1=\pi_2\), then we can simplify \(p_1-p_2\):
\[p_1-p_2\sim N\left(\pi_1-\pi_2,\pi(1-\pi)\left(\frac{1}{n_1}+\frac{1}{n_2}\right)\right)\]
We would like to simplify this even further however we do not know what \(\pi\) is, so we will have to estimate it using the total proportion:
\[\hat{p}=\frac{X_1+X_2}{n_1+n_2}\]
For a test at an \(\alpha\) significance level against \(H_1:\pi_1\ne \pi_2\) we reject \(H_0\) if:
\[|z|=\left|\frac{p_1-p_2}{\sqrt{\hat{\pi}(1-\hat{p})\left(\frac{1}{n_1}+\frac{1}{n_2}\right)}}\right|>z_{1-\alpha/2}\]
\section{Correlation}
In the Autumn semester we learned about bivariate distributions. We want to consider pairs of data \((x_i,y_i)\) from \(X\) and \(Y\) to see if there is a possible relationship between them. A descriptive measure of the strength of the linear relationship between two sets of observations is the sample covariance:
\[s_{XY}=\sum^n_{i=1}(x_i-\bar{x})(y_i-\bar{y})\]
\[=\frac{1}{n-1}\left(\left(\sum^n_{i=1}x_iy_i\right)-\frac{1}{n}\left(\sum^n_{i=1}x_i\right)\left(\sum^n_{i=1}y_i\right)\right)\]
This is an estimate of the population covariance:
\[\text{cov}(X,Y)=\sigma_{XY}=E[XY]-E[X]E[Y]\]
\[E[s_{XY}]=\sigma_{XY}\]
The sample covariance depends on the units of measurement so it is not always the best choice for looking at relationships. For this we can consider the sample correlation coefficient:
\[r=\frac{s_{XY}}{s_Xs_{Y}}\]
Where \(s_X,\,s_Y\) are the sample standard deviations of \(X\) and \(Y\) respectively.
\subsection*{Properties of sample correlation}
\begin{itemize}
    \item The correlation is bounded: \(-1\leq r\leq 1\).
    \item The larger the value of \(|r|\), the stronger the linear association.
    \item A negative \(r\) value indicates negative association.
    \item \(r=1\) is perfect positive correlation, meaning both data sets lie on the same straight line.
    \item \(r=-1\) is perfect negative correlation.
    \item if \(r=0\) there is no linear correlation between the datasets.
\end{itemize}
The sample correlation is known as Pearson's correlation coefficient or the product moment correlation coefficient. This is the most common correlation coefficient and it estimates:
\[\rho=\frac{\sigma_{XY}}{\sigma_X\sigma_Y}\]
A nice property of correlation is that it is independent of units of measurement. This because any chance to a unit would just rescale the data rather than change the underlying distribution.
\subsubsection*{A note on causality}
Be careful when talking about correlation and causality. know that correlation does not imply causality and is a weaker condition.
\subsection*{Hypothesis testing for correlation}
There are 2 main method for hypothesis testing for correlation:
\begin{itemize}
    \item You can perform a t-test. This is the best kind of test to perform when we know we have linear association and our data is normal or approximately normal (i.e. bivariate normal)
    \[\binom{X}{Y}\sim N\left(\binom{\mu_X}{\mu_Y},\,\begin{pmatrix}
        \sigma_X^2 & \rho\sigma_X\sigma_Y \\
        \rho\sigma_X\sigma_Y & \sigma_Y^2
    \end{pmatrix}\right)\]
    \item You can perform a non-parametric test. This is most suitable for non-linear association or we cannot make assumptions about the normality of the underlying data.
\end{itemize}
\subsubsection*{T-test}
In this test our hypotheses are most often:
\[H_0: \rho=0\]
\[H_1: \rho \ne 0\]
Although the alternate hypothesis can sometimes be one sided. We then need to assume our data is normal or can be approximated as normal. We choose our test statistic to be:
\[t=r\sqrt{\frac{n-1}{1-r^2}}\]
Where \(r=\frac{s_{XY}}{s_{X}s_{Y}}\). Under \(H_0\) we have \(t\sim t_{n-2}\). Then we can compare critical values like usual.
\subsubsection*{Non-parametric test}
If we cannot assume normality or the relationship is non-linear then we can perform a non-parametric test. We do however, require that the relationship is monotonic. Consider \(n\) pairs of data \((x_i,y_i)\) from \(X,Y\). We are going to look at the difference of ranks for each of these data points. Then we can calculate Spearman's rank order correlation coefficient, \(r_s\):
\[r_s=1-\frac{6\sum^n_{i=1}d_i^2}{n(n^2-1)}\]
Where \(d_i\) is the difference in rank for pair \(i\). We can then compare \(r_s\) against the critical values found in statistical tables.
\section{Goodness of fit}
In a goodness of fit test we are trying to match up data to a distribution. We want to check if the distribution fits the observed data well or not. For a hypothesis test of this form we have:
\[H_0: \text{Model fits well}\]
\[H_1: \text{Model does not fit}\]
with test statistic:
\[\chisq=\sum_i\frac{(O_i-E_i)^2}{E_i}\]
Where \(O_i\) is observed frequency for data point \(i\) and \(E_i\) is the expected frequency under \(H_0\). we reject \(H_0\) at the \(\alpha\) significance level if:
\[\chisq\geq \chi^2_{\nu,1-\alpha}\]
Where \(\nu\) is the degrees of freedom of the test. This is \(\nu=\#\text{Categories}-\#\text{Estimated parameters}-1\). Note that \(\chisq\) does not exactly follow a \(\chi^2\) distribution but rather just approximates it in a similar way to how the central limit theorem works. \(E_i\) should be greater than or equal to \(5\) for exactly this reason, we want to ensure we have an accurate approximation. If the expected values are not like this we need to combine categories until they all are.
\subsection*{Example}
let \(X\) be the number of storms in a month in some city. We have observed frequency data:
\begin{table}[H]
    \centering
    \begin{tabular}{c|c|c|c|c|c|c|c}
        \(X\) & 0 & 1 & 2 & 3 & 4 & 5 & \(\geq 6\) \\ \hline
        \(O_i\) & 22 & 37 & 20 & 13 & 6 & 2 & 0
    \end{tabular}
\end{table}
We are going to test if this data fits well to a Poisson distribution with \(\alpha=0.1\):
\[H_0: X\sim \text{Po}(\lambda)\]
\[H_1: \text{Poisson is not a good fit}\]
We need to estimate \(\lambda\) first however, so we look at the standard unbiased estimator:
\[E[\bar{x}]=\lambda\]
\[\bar{x}=\hat{\lambda}\]
In this specific case we have \(\bar{x}=1.5\) so we will take that for \(\lambda\). Then we can fill in the expected values, combining categories where needed:
\begin{table}[H]
    \centering
    \begin{tabular}{c|c|c|c|c|c}
        \(X\) & 0 & 1 & 2 & 3 & \(\geq4\) \\ \hline
        \(O_i\) & 22 & 37 & 20 & 13 & 8 \\
        \(E_i\) & 22.3 & 33.5 & 14.9 & 12.6 & 6.6 \\
        \((O_i-E_i)\) & -0.3 & 3.5 & 5.1 & 0.4 & 1.4 
    \end{tabular}
\end{table}
\[\chisq =\sum_i\frac{(O_i-E_i)^2}{E_i}=\frac{(-0.3)^2}{22.3}+\frac{(3.5)^2}{33.5}+\dots+\frac{(1.4)^2}{6.6}\approx 1.7156\]
The degrees of freedom in this case are \(\nu=5-1-1=3\), this comes from combining categories as well as estimating \(\lambda\). We can also find our critical value:
\[\chi^2_{3,0.9}=6.25\]
\[6.25\geq 1.7156\]
So we do not reject \(H_0\).
\subsection*{Contingency tables and independence}
We can also test for independence. We can plot a two way table called a contingency table that shows the observed values from both categories. We estimate the probability of being in any one of the groups by:
\[\hat{p}=\frac{\text{Total in category}}{\text{Total sample size}}\]
We can then use properties of independence to calculate the expected values for each category. In particular:
\[P[E\cap F]=P[E]P[F]\]
We can apply this in our situation and see that if we are looking at the expected number for being in the overlap of category \(A\) and \(B\) we have:
\[E_i=n\cdot P[A]P[B]\]
We can construct a new table of expected values using this formula and from there a table of differences. If we sum up all of these results we have our test statistic, note that this is exactly the same formula as before we have just done it in a slightly different way:
\[\chisq = \sum_i\frac{(O_i-E_i)^2}{E_i}\]
Finally we can compare against our critical value:
\[\chisq\geq \chi^2_{\nu,1-\alpha}\]
Where \(\nu=(\#\text{Rows}-1)(\#\text{Cols}-1)\).
\section{Linear regression}
\subsection*{Introduction}
Consider a linear dataset of two variables. We are interested in the best relationship we can find between them. Our main motivation for this is that once we find this relationship, we can use it to make predictions about other pieces of data.
\subsection*{Running example}
Whilst looking at linear models and regression we will use the example of the relationship between GDP per capita and life expectancy across 6 countries. We want to estimate life expectancy based upon GDP per capita:
\begin{table}[H]
    \centering
    \begin{tabular}{c|c|c}
        \textbf{Country} & \textbf{x} & \textbf{y} \\ \hline 
        Ukraine & \(8.2\) & \(67.9\) \\
        UAE & \(11.1\) & \(78.7\) \\
        UK & \(10.6\) & \(79.4\) \\
        US & \(10.8\) & \(78.3\) \\
    ...
    \end{tabular}
\end{table}
In particular we may wish to predict the life expectancy in South Africa with a GDP per capita of \(9.03\).
\subsection*{Linear models}
If we have a linear relationship then we can construct a model as an equation of the form:
\[y_i=\beta_0+\beta_1x_i+\varepsilon_i,\qquad i=1,\dots,n\]
Where \(\beta_0,\,\beta_1\) are the regression coefficients and \(\varepsilon_i\) are unobservable errors with a zero mean and variance, \(\sigma^2\). \(\beta_0\) is the intercept and \(\beta_1\) is the slope. This model specifies a mean:
\[E[Y_i]=\beta_0+\beta_1x_i\]
Which is a linear function in the \(x\)-direction with an additional error in the \(y\)-direction. There is no error in the \(x\)-direction because all \(x_i\)'s are assumed to be known. Linear regression is suitable for prediction of \(Y_i\) given \(x_i\). A natural question that now arises is how to pick \(\beta_0\) and \(\beta_1\)?
\subsection*{Model assumptions}
These linear models make some assumptions, namely:
\begin{itemize}
    \item Errors have zero mean, \(E[\varepsilon_i]=0\)
    \item Errors are uncorrelated, \(\text{cov}(\varepsilon_i,\varepsilon_j)=0,\,i\ne j\)
    \item Errors have the same variance, \(\Var{\varepsilon_i}=\sigma^2\)
\end{itemize}
Then we can estimate \(\beta_0\) and \(\beta_1\) by the principle of least squares. Note that we do not need to have normally distributed errors but if we do then we can perform hypothesis testing and all other requirements will be met. As an extension we sometimes say that all \(\varepsilon_i\overset{\text{i.i.d}}{\sim}N(0,\sigma^2)\).
\subsection*{Principle of least squares}
We wish to minimise our errors:
\[\varepsilon_i=y_i-\beta_0-\beta_1x_i\]
So we choose \(\beta_0,\,\beta_1\) to minimise the sum of the squares of the errors:
\[S=\sum^n_{i=1}\varepsilon_i^2=\sum^n_{i=1}(y_i-\beta_0-\beta_1x_i)^2\]
The estimators of \(\beta_0,\,\beta_1\) that minimise \(S\) are:
\[\hat{\beta}_0=\bar{y}-\hat{\beta}_1\bar{x}\]
\[\hat{\beta}_1=\frac{s_{xy}}{S_x^2}\]
\subsection*{Derivation}
\[S=\sum^n_{i=1}(y_i-\beta_0-\beta_1x_i)^2\]
We wish to minimise so we are looking for turning points:
\[\frac{\partial S}{\partial \beta_0}=\sum^n_{i=1}2(y_i-\beta_0-\beta_1x_i)(-1)\]
\[=-2n\bar{y}+2n\beta_0+2n\beta_1\bar{x}\]
\[\frac{\partial S}{\partial\beta_1}=\sum^n_{i=1}2(y_i-\beta_0-\beta_1x_i)(-x_i)\]
\[=-2\sum^n_{i=1}x_iy_i+2n\beta_0\bar{x}+2\beta_1\sum^n_{i=1}x_i^2\]
We want to set \(\frac{\bar{x}}{2}S_{\beta_0}=0\) and \(-\frac{1}{2}S_{\beta_1}=0\):
\[-n\bar{y}\bar{x}+n\bar{x}\beta_0+\beta_1n\bar{x}^2=\sum^n_{i=1}x_iy_i-n\bar{x}\beta_0-\beta_1\sum^n_{i=1}x_i^2\]
\[\left(\sum^n_{i=1}x_iy_i-n\bar{y}\bar{x}\right)-\beta_1\left(\sum^n_{i=1}x_i^2-n\bar{x}^2\right)=0\]
\[(n-1)s_{xy}-(n-1)s_x^2\beta_1=0\]
\[\hat{\beta}_1=\frac{s_{xy}}{s_x^2}\]
\[-\bar{y}+\beta_0+\beta_1\bar{x}=0\]
\[\hat{\beta}_0=\bar{y}-\hat{\beta}_1\bar{x}\]
\subsection*{Applying the estimators to our population example}
\[n=6\quad\bar{x}=9.6\quad \bar{y}=74.65\]
\[s_{xy}=8.358\quad s_x^2=2.388 \quad s_y^2=31.283\]
\[\hat{\beta}_1=\frac{s_{xy}}{s_{x}^2}=\frac{8.358}{2.388}=3.5\]
\[\hat{\beta}_0=74.65-3.5(9.6)=41.05\]
\(\beta_0\) represents the expected life population if a country had a GDP per capita of \(0\). \(\beta_1\) represents how much the life expectancy of a country increases per unit increase of GDP per capita. Consider our example of South Africa:
\[\hat{Y}_{SA}=\hat{\beta}_0+\hat{\beta}_1x_{SA}\]
\[\hat{Y}_{SA}=41.05+3.5(9.03)=72.655\approx72.7\]
\subsection*{Model evaluation}
The natural question that comes up here is "how good is our linear model". When we fit a model we are mostly only interested in a small range of values so we can inspect the residuals:
\[r_i=y_i-\hat{y}_i\]
The residual plot is found by plotting the values \(r_i\) against \(\hat{y}_i\). We want to see no clear pattern in the residuals for our model to be good.
\subsubsection*{Justifying \(\hat{\beta}_0\) and \(\hat{\beta}_1\)}
It can be seen that:
\[\beta_0=E[\hat{\beta}_0]\]
\[\beta_1=E[\hat{\beta}_1]\]
So \(\hat{\beta}_0\) and \(\hat{\beta}_1\) are unbiased estimators. Note that the better range of \(x\) values the we have the better model fit we will get.
\subsubsection*{Estimating \(\sigma^2\)}
We can estimate the \(\sigma^2\) variance of our error terms with:
\[\hat{\sigma^2}=\frac{1}{n-2}\sum^n_{i=1}(y_i-\hat{y}_i)^2=\frac{1}{n-2}\sum^n_{i=1}r_i^2\]
A larger \(\hat{\sigma^2}\) would indicate a worse model fit. Alternatively:
\[\sum^n_{i=1}r_i^2=(n-1)s_x^2+\hat{\beta}_1^2(n-1)s_x^2-2\hat{\beta}_1(n-1)s_{xy}\]
Since \(s_{xy}=\hat{\beta}_1s_x^2\):
\[\hat{\sigma^2}=\frac{(n-1)}{(n-2)}\left(s_y^2-\hat{\beta}_1^2s_x^2\right)\]
\subsection*{Inference about linear regression}
If we look at our running example we a fitted regression line:
\[\hat{y}=41.05+3.5x\]
But now there are some questions that naturally arise such as:
\begin{itemize}
    \item How accurate is the slope?
    \item How good is any particular guess?
    \item What are the confidence intervals for individual countries?
\end{itemize}
To do inference we need to include the extra assumption that we previously left out, we need the distribution of each error to normal:
\[\varepsilon_i\sim N(\mu,\sigma^2)\]
Consequently, we have that
\[Y_i|x_i\sim N(\beta_0+\beta_1x_i,\sigma^2)\]
\subsection*{Checking normality}
We can check if the errors are distributed normally or approximately normally using a quantile-quantile plot (Q-Q plot). These are graphs which theoretical quantiles plotted on the x-axis and residual quantiles plotted on the y-axis. Theoretical quantiles are calculated using the normal c.d.f, in particular we have:
\[\Phi(x_i)=\frac{i-\frac{1}{2}}{n}\qquad i=1,2,...,n\]
Where the values \(x_i\) are the theoretical quantiles. For the residual quantiles, we calculate the residuals as normal and then normalise them:
\[r_i=\frac{y_i-\hat{y}_i}{\sqrt{\sigma}}\]
Then we sort the residuals which we denote: \(r_{(i)}\), the Q-Q plot is then the points \((x_i,r_{(i)})\) plotted. If we have normal errors then the line through this graph should have a very strong positive correlation.
\subsection*{Testing evidence for regression}
Since we now impose the requirement that the error terms are distributed normally we can start to do some inference on the regression coefficients. In particular we have:
\[\hat{\beta}_1=\frac{s_{xy}}{s_x^2}\sim N\left(\beta_1,\frac{\sigma^2}{(n-1)s_x^2}\right)\]
We can use this distribution as a basis to ask inference questions about \(\beta_1\). Standardising gives us:
\[z=\frac{\hat{\beta}_1-\beta_1}{\sigma/\sqrt{(n-1)s_x^2}}\sim N(0,1)\]
Typically we do not actually know what \(\sigma^2\) is so we also have to estimate this, which means our test is normally:
\[t=\frac{\hat{\beta}_1-\beta_1}{\hat{\sigma}/\sqrt{(n-1)s_x^2}}\sim t_{n-2}\]
Which we can then use to construct confidence intervals for the mean:
\[\hat{\beta}_1\pm t_{n-2,1-\alpha/2}\frac{\hat{\sigma}}{\sqrt{(n-1)}s_x^2}\]
If \(\beta_1=0\) then there is no linear relationship. Using this fact, we can perform a hypothesis test to test for the existence of regression:
\[H_0\!: \beta_1=0\]
\[H_1\!: \beta_1\ne 0\]
Assuming \(H_0\), we reject the null hypothesis if:
\[|t|=\left|\frac{\hat{\beta}_1}{\hat{\sigma}/\sqrt{(n-1)s_x^2}}\right|\geq t_{n-2,1-\alpha/2}\]
\subsection*{Confidence intervals for the mean}
The variance of the fitted line \(Y_i=\hat{\beta}_0+\hat{\beta}_1X\) where \(X=x_0\) is:
\[\Var{\hat{Y}|X=x_0}=\Var{\hat{\beta}_0+\hat{\beta}_1x_0}\]
\[=\Var{\hat{\beta}_0}+x_0^2\Var{\hat{\beta}_1}+2x_0\text{cov}(\hat{\beta}_0,\hat{\beta}_1)\]
\[=\frac{\sigma^2\sum x_i^2}{n(n-1)s_x^2}+\frac{x_0^2\sigma^2}{(n-1)s_x^2}-\frac{2x_0\sigma^2\bar{x}}{(n-1)s_x^2}\]
\[=\frac{\sigma^2}{(n-1)s_x^2}\left(\frac{\sum x_i^2}{n}+x_0^2-2x_0\bar{x}\right)\]
We can use the sample variance formula and simplify this as:
\[\Var{\hat{Y}|X=x_0}=\sigma^2\left(\frac{1}{n}+\frac{(x_0-\bar{x})^2}{(n-1)s_x^2}\right)\]
We can then use this variance to construct a confidence interval for the mean of \(Y\) at \(x_0\):
\[(\hat{\beta}_0+\hat{\beta}_1x_0)\pm(t_{n-2,1-\alpha/2})\left(\hat{\sigma}\sqrt{\frac{1}{n}+\frac{(x_0-\bar{x})^2}{(n-1)s_x^2}}\right)\]
These confidence intervals form a tight band around the point \(x_0\) and are the smallest when \(\bar{x}=x_0\), as you get further away from the mean they are less and less accurate.
\subsection*{Prediction intervals for particular \(Y\)}
The variance for any particular predicted \(\hat{y}\) value is the same as the error that comes from the mean with the addition of some built in uncertainty:
\[\Var{\hat{y}|X=x_0}=\Var{\hat{Y}|X=x_0}+\Var{\varepsilon}\]
\[=\sigma^2\left(1+\frac{1}{n}+\frac{(x_0-\bar{x})^2}{(n-1)s_x^2}\right)\]
Notice that this always has some non-zero variance, as opposed to the mean variance which can tend to \(0\) with enough sample points. We can construct a confidence interval exactly like before using this variance. The width of this interval is smallest when \(\bar{x}=x_0\).
\subsection*{ANOVA}
Analysis of variance is another way of thinking about regression. The key question we are trying to answer with ANOVA is how much of the variability in the observed data can be explained by the regression model? We start with the total sum of squares:
\[S(\text{total)}=\sum^n_{i=1}(y_i-\bar{y})^2=(n-1)s_y^2\]
The total sum of squares is a measure of the total variability in the data, we can use a standard mathematical trick to split this sum into two parts:
\[\sum^n_{i=1}(y_i-\bar{y})^2=\sum^n_{i=1}(y_i-\hat{y}_i)^2+\sum^n_{i=1}(\hat{y}_i-\bar{y})^2\]
Where:
\[\sum^n_{i=1}(y_i-\hat{y}_i)^2\]
Is called the regression sum of squares and:
\[\sum^n_{i=1}(\hat{y}_i-\bar{y})^2\]
Is called the residual sum of squares. Essentially the regression sum of squares explains how much of the variability is explained by the regression model and the residual sum of squares is everything left over. A measure of how useful the regression model is the coefficient of determination:
\[R^2=\frac{S(\text{regression)}}{S(\text{total})}\]
It is important to consider \(R^2\) values in context, because what might sounds like a very strong \(R^2\) value might actually not be when thinking about desired tolerances or confidence intervals.
\section*{Stochastic modelling}
\subsection*{Discrete time stochastic process}
A stochastic process is a family of random variables \(\{X_t:t\in T\}\) indexed by some parameter set \(T\). Usually \(t\) is called a time parameter. When \(T\) is countable \(\{X_t:t\in T\}\) is called a discrete-time process. When \(T=[0,\infty)\) or \((a,b)\) we call it a continuous-time process. An example of a stochastic process we might be interested in is the temperature at a given location at a given point.
\subsection*{State space}
The set \(S\) in which \(X_n\) takes value is called the state space of the process. We assume \(S\) is countable. We require that the random variables share a common state space
\subsubsection*{Examples}
\begin{itemize}
    \item Roll a dice repeatedly. Let \(X_n\) be the value of the nth roll:
        \[S=\{1,2,3,4,5,6\}\]
    \item Toss a coin. Let \(X_n\) be the number of heads so far:
        \[S=\{0,1,2,\dots\}\]
    Note that in this case there is some dependence on our outcome. In particular:
        \[m<n\implies X_m\leq X_n\]
    \item Gamblers ruin: You got to casino with a set amount of money and bet the same amount of money repeatedly on the red roulette spin. You stop if you meet a target amount of money or you go bust. One such state space could look like:
    \[S=\{0,10,20,\dots,2000\}\]
    \[X_0=1000\]
\end{itemize}
\subsection*{Sample path}
A sample path is a sequence of realised values of a stochastic process
\subsubsection*{Example}
Roll a die. Start on a score of \(X_0=0\) and keep rolling. \(X_n\) is the sum of the first \(n\) dice rolls. The state space is:
\[S=\{0,1,2,\dots\}\]
and we can explore what some paths may look like:
\begin{table}[H]
    \centering
    \begin{tabular}{c|c c c c c c c}
        \(n\) & 1 & 2 & 3 & 4 & 5 & 6 & \(\longrightarrow\) \\ \hline
        \(X_n(\omega_1)\) & 4 & 7 & 11 & 12 & 13 & 17 \\ 
        \(X_n(\omega_2)\) & 6 & 12 & 14 & 17 & 23 & 27 \\
        \(X_n(\omega_3)\) & 5 & 11 & 16 & 19 & 20 & 25 \\
        \(X_n(\omega_4)\) & 5 & 9 & 14 & 19 & 24 & 29 \\
        \(X_n(\omega_5)\) & 6 & 12 & 17 & 20 & 22 & 24 \\
        \(\downarrow\)
    \end{tabular}
\end{table}
Where each \(X_n(\omega_i)\) is a different sample path.
\subsection*{Markov process}
For a discrete-time stochastic process \(\{X_n:n=0,1,2,...\}\), the Markov property says that for all \(n=0,1,2,...\) and for all \(x_0,x_1,...,x_{n+1}\in S\) we have:
\[P[X_{n+1}=x_{n+1}\mid X_0=x_0, X_1=x_1,\dots,X_n=x_n]=P[X_{n+1}=x_{n+1}\mid X_n=x_n]\]
A stochastic process satisfying the Markov property is known as a Markov process. Essentially what this says is that the future is only dependent on the present and nothing in the past. Looking back at the examples from before we can see that they are all examples of Markov processes, the first dice rolling one in particular is a trivial Markov process.
\subsubsection*{Time homogeneity}
A Markov process is called time homogenous if \(\forall n,k\geq 0\) and \(x,y\in S\):
\[P[X_{n+k}=y\mid X_n=x]=P[X_k=y\mid X_0=x]\]
In this case all that matters is the time step gap between events not when they actually happen.
\subsection*{Simple random walk}
Consider a walk on the integers, numbered left to right. We choose a starting point and the use a probability distribution to decide how to move along the line. We move and record our new position and then repeat. \(X_0=x\) represents our starting position and \(X_n\) is where we end up after \(n\) steps.
\subsubsection*{Example - Ant escape}
Imagine an ant in a tunnel. The ant wants to escape the tunnel and to do so it must move 10 units in a given direction. At each step the ant moves 1 unit towards or away from the exit. The ant needs to escape within 100 steps to survive. What we want to know is what is the probability that the ant escapes within those 100 steps given a probability distribution telling us how frequently the ant moves the correct way.
\[X_0=0\]
\[Z=\begin{cases}
    +1 & \text{Probability }p \\
    -1 & \text{Probability }q=1-p
\end{cases}\]
The position of the ant after \(n\) steps is:
\[X_n=X_0+\sum^n_{i=1}Z_i=\sum^n_{i=1}Z_i\]
\subsubsection*{Example - Gamblers ruin, roulette}
A roulette wheel has 37 numbers: 1 green, 18 red and 18 black. Start with £\(1000\) and keep placing £\(10\) bets on red; stopping when you reach £\(2000\) or go bust (£\(0\)). 
\begin{align*}
    &\text{Probability of winning }=\frac{18}{37}=0.486 \\
    &\text{Probability of losing }=\frac{19}{37}=0.514
\end{align*}
This is a random walk with a random stopping time. We have \(X_0=1000\) and our steps are distributed according to:
\[Z=\begin{cases}
    +10 & p=0.468 \\
    -10 &  p=0.514
\end{cases}\]
We stop at \(\tau\):
\[\tau=\min\{n:X_n=0\text{ or }X_n=2000\}\]
We wish to ask what is the probability that the gambler is successful?
\[P[X_\tau=2000]\]
In general it may be beneficial to re-parametrise the model to work with steps of size 1 unit.
\subsection*{Gamblers ruin - General solution}
The formulation of the problem is as follows:
\begin{enumerate}
    \item An individual enters the casino with \(x\) units of wealth.
    \item The individual places 1 unit bets with probability \(p\) such that if they win, their wealth increases by 1 unit and if they lose it decreases by 1 unit.
    \item The individual stops when they have 0 units of wealth or they have hit their target goal of \(N\) units of wealth.
\end{enumerate}
Let \(E_k\) be the vent that the gambler leaves with \(N\) units of wealth given that they started with \(k\) units. Let \(\omega_k=P[E_k]\), we can calculate \(\omega_k\) as follows:
\begin{enumerate}
    \item Write down \(\omega_k\) in terms of \(\omega_{k+1}\) and \(\omega_{k-1}\)
    \item Use \(p+q=1\) to find a difference equation
    \item Substitute in boundary conditions
    \item Start with the case \(p=q=0.5\) and then look at \(p\ne q\)
\end{enumerate}
Let \(A=\{\text{First bet success}\}\), then:
\[\omega_k=P[E_k]=P[E_k\mid A]P[A]+P[E_k\mid A^c]P[A^c]\]
\[=P[E_{k+1}]P[A]+P[E_{k-1}]P[A^c]\]
\[\omega_k=p\omega_{k+1}+q\omega_{k-1}\]
\[\implies (p+q)\omega_k=p\omega_{k+1}+q\omega_{k-1}\]
\[\implies q(\omega_k-\omega_{k-1})=p(\omega_{k+1}-\omega_k)\]
\[\implies \omega_{k+1}-\omega_k=\frac{q}{p}(\omega_k-\omega_{k-1})\]
Using \(\omega_0=0\):
\[\omega_{k+1}-\omega_k=\left(\frac{q}{p}\right)^k\omega_1\]
We also know that we can write:
\[\omega_{k+1}-\omega_1=(\omega_{k+1}-\omega_k)+(\omega_k-\omega_{k-1})+\cdots + (\omega_2-\omega_1)\]
\[=\sum^k_{j=1}(\omega_{j+1}-\omega_j)\]
\[=\sum^k_{j=1}\left(\frac{q}{p}\right)^jw_1\]
Then:
\[\omega_{k+1}=\omega_1\sum^k_{j=0}\left(\frac{q}{p}\right)^j\]
\[\implies \omega_{k+1}=\begin{cases}
    (k+1)\omega_1 & p=q=0.5 \\
    \frac{1-\left(\frac{q}{p}\right)^{k+1}}{1-\frac{q}{p}}\omega_1 & p\ne q
\end{cases}\]
Finally, using \(\omega_N=1\), if \(p=q=0.5\) then:
\[1=\omega_N=N\omega_1\implies \omega_1=\frac{1}{N}\implies \omega_k=\frac{k}{N}\]
And if \(p\ne q\):
\[1=\omega_N=\frac{1-\left(\frac{q}{p}\right)^{N}}{1-\frac{q}{p}}\omega_1\implies\omega_1=\frac{1-\left(\frac{q}{p}\right)}{1-\left(\frac{q}{p}\right)^N}\]
\[\implies \omega_k=\frac{1-\left(\frac{q}{p}\right)^k}{1-\left(\frac{q}{p}\right)^N}\]
\subsubsection*{Applied to the roulette example}
\[k=x=100\]
\[N=200\]
\[p=\frac{18}{37}\implies \omega_k=\frac{1-\left(\frac{q}{p}\right)^{100}}{1-\left(\frac{q}{p}\right)^{200}}\]
\[\implies P[E_k]=0.004466\]
So we can see how the probability of winning shrinks hugely over the hundreds of steps.
\section*{Financial modelling}
One of the simplest financial models is the binomial model. Given that a stock is currently valued \(S_0=s\), the probability of an up move is \(p\) and the new value is \(u\times s\) for \(u>1\) and the probability of a down move is \(q=1-p\) and the new value is \(d\times s\) for \(0<d<1\). \(S_n\) is the value of the stock after \(n\) steps.
\subsection*{Option pricing}
A (European) call option allows an individual to buy a stock at a pre-determined price (strike price) \(k\), at a specified time in the future. A (European) put option allows an individual to sell a stock at a strike price at a specified time in the future. Note that you have the option to buy/sell at the strike price but not the obligation. A key question now is what is a fair price for an option?
\subsubsection*{Call option pricing}
Suppose you purchase a call option with strike price \(k\).
\begin{itemize}
    \item If \(S_N> k\) it is beneficial to exercise the option.
    \item If \(S_N\leq k\) it is not beneficial.
\end{itemize}
Your expected profit is:
\[C=E[\max\{S_n-k,0\}]\]
\subsubsection*{Put option pricing}
\begin{itemize}
    \item If \(S_N\geq k\) it is not beneficial to exercise the option.
    \item If \(S_N<k\) it is beneficial.
\end{itemize}
Your expected profit is:
\[P=E[\max\{k-S_N,0\}]\]
\subsubsection*{Example}
Suppose that at a time \(0\), a stock is worth £80.
\[S_0=80\]
Every month the stock goes up by £4 with probability \(0.6\) or it goes down by £3 with probability \(0.4\). Somebody offers to buy our stock in 3 months for £85 and we wish to work out if this is a good deal. Let \(U\) be the number of up moves:
\[U\sim N(3,0.6)\]
Our expected profit is:
\[P=E[\max\{k-S_3,0\}]\]
So we wish to examine the distribution of \(S_3\):
\begin{table}[H]
    \centering
    \begin{tabular}{c|c c c c}
        ...
        \(S_3\) & 71 & 78 & 85 & 92 \\
        \(k-S_3\) & 14 & 7 & 0 & -7 \\
        \(P(U=u)\) & \(0.4^3\) & \(3(0.4)^2(0.6)\) & \(3(0.4)(0.6)^2\) & \(0.6^3\)
    \end{tabular}
\end{table}
\[P=(0.4)^3\times 14+3(0.4)^2(0.6)\times 7=2.912\]
In this case we are expecting on average to turn a £\(2.91\) profit from a sale like this so we should take their offer.
\section*{PGFs and Branching Processes}
\subsection*{Definition and motivation}
Let \(X\) be a discrete random variable taking values in \(\{0,1,2,...\}\). The probability generating function of \(X\) is denoted \(\phi_X\) and is a function of the real variable \(s\) defined by:
\[\phi_X(s)=E[s^X]=\sum_{k=0}^\infty p_X(k)s^k\]
for all values of \(s\) such that this sum is finite. Notice that this is necessarily defined for \(s\in[-1,1]\) since:
\[\sum^\infty_{k=0} |p_X(k)s^k|\leq\sum^\infty_{k=0}p_X(k)=1\]
PGF's are useful for studying the properties of random variables and stochastic processes.
\subsubsection*{Example - Poisson random variable}
Suppose \(X\sim \text{Po}(\lambda)\), then the pgf of \(X\) is given by:
\[\phi_X(s)=E[s^X]=\sum^\infty_{k=0}p_X(s)s^k\]
\[=\sum^\infty_{k=0}\exp(-\lambda)\frac{\lambda^k}{\lambda!}s^k\]
\[=\exp(-\lambda)\sum^\infty_{k=0}\frac{(\lambda s)^k}{k!}\]
\[=\exp(-\lambda)\exp(\lambda s)\]
\[=\exp([s-1]\lambda)\]
\subsection*{Moments of the pgf and special cases}
Let \(\phi^{(n)}_X(s)\) denote the \(n^{th}\) derivative of \(\phi_X(s)\). Then:
\[\phi^{(n)}_X(1)=E[X(X-1)(X-2)\cdots(X-n+1)]\]
for \(n=1,2,\dots\). Notice that there are some special cases of this:
\begin{itemize}
    \item \(E[X]=\phi'_X(1)\)
    \item \(\Var{X}=\phi''_X(1)+\phi'_X(1)-[\phi'_X(1)]^2\)
\end{itemize}
\subsection*{Calculating probabilities from PGFs}
The pmf of a random variable \(X\) is uniquely determined by its pgf with:
\[p_X(n)=\frac{\phi^{(n)}_X(0)}{n!}\]
Alternatively and perhaps more usefully, \(p_X(n)\) is given by the coefficient of \(s^n\) in the Maclaurin expansion of \(\phi_X(s)\).
\subsection*{Sums of random variables}
Let \(X_1,X_2,\dots,X_n\) be independent random variables and:
\[Z=X_1+X_2+\cdots+X_n\]
then:
\[\phi_Z(s)=\prod^n_{i=1}\phi_{X_i}(s)\]
This is particularly useful for certain distributions such as the Poisson distribution with:
\[X\sim \text{Po}(\lambda),\quad Y\sim\text{Po}(\mu)\]
\[Z=X+Y\]
\[Z\sim \text{Po}(\lambda+\mu)\]
\subsection*{Branching processes}
Branching processes help us answer predicting future questions such as:
\begin{itemize}
    \item What will the population of the UK be in 2100?
    \item How likely is it that I will have a (great-great-....) grandson with my family name?
    \item If we introduce a disease into a population how likely is it that it will take off?
\end{itemize}
Let \(X_n\) denote the number of individuals alive in the \(n^{th}\) generation of the branching process, where \(X_0\) denotes the initial starting size. We very often assume \(X_0=1\). The number of offspring an individual has is distributed according to a discrete offspring distribution \(Z\), independent of the offspring of other individuals. For \(n\geq1\):
\[X_n|\{X_{n-1}=j\}\overset{D}{=}\sum^j_{i=1}Z_i\]
Or equivalently:
\[X_n=\begin{cases}
    \sum^{X_n-1}_{i=1}Z_{n,i}& X_{n-1}>0 \\
    0 & X_{n-1}=0
\end{cases}\]
Where \(Z_{n,i}\) is the number of offspring of the \(i^{th}\) member of the \((n-1)^{th}\) generation. A useful tool for exploring branching processes is pgfs. Let \(p_k=P(Z=k)\) be the probability an individual has \(k\) offspring and let \(\textbf{p}=(p_0,p_1,\dots)\). Now let:
\[\mu=E[Z]=\sum^\infty_{k=0}kp_k\]
The branching process is said to be,
\begin{itemize}
    \item Sub-critical if \(\mu<1\)
    \item Critical if \(\mu=1\)\
    \item Super-critical if \(\mu>1\)
\end{itemize}
\subsection*{Law of total expectation}
Suppose that \(X\) and \(Y\) are two random variables in the same probability space and that \(E[X]\) is defined, then:
\[E[X]=E[E[X\mid Y]]\]
Suppose \(Y\) takes values on \(\{0,1,2,...\}\) then:
\[E[X]=\sum^\infty_{n=0}E[X\mid Y=n]P[Y=n]\]
Starting from \(X_0=1\), for \(n=1,2,...\)
\[E[X_n]=\mu^n\]
and immediate consequence of this is:
\[E[X_n]=\mu^n\to\begin{cases}
    \infty& \mu>1\text{ (Super-critical)} \\
    1 & \mu=1 \text{ (Critical)} \\
    0 & \mu<1 \text{ (Sub-critical)}
\end{cases}\]
as \(n\to\infty\).
\subsubsection*{Progeny of a branching process}
The total progeny, \(T\) of a branching process is the total number of individuals ever alive in the branching process:
\[T=\sum^\infty_{j=1}X_j\]
\[E[T]=\begin{cases}
    \frac{1}{1-\mu} & \mu<1 \\
    \infty & \mu\geq 1
\end{cases}\]
A branching process is said to go extinct if \(X_n=0\) for some \(n>0\) Note that once \(X_n=0\) then all \(X_{n+i}=0\) for any \(i\geq 0\). If \(p_0=P(Z=0)=0\) then it is not possible for the population to go extinct.
\subsection*{PGFs and branching processes}
For \(0\leq s\leq 1\), let \(\phi_Z(s)\) be the pgf of \(Z\). The extinction probability for a branching process with offspring distribution \(Z\) is the smallest non-negative solution in \(0\leq s\leq 1\) such that:
\[s=\phi_Z(s)\]
Furthermore, if \(\mu<1\) then we are certain that the population will go extinct at some point in the future. If \(\mu>1\) we are not guaranteed that the population will go extinct although it still can.
\subsubsection*{Observations of extinction criteria}
\begin{itemize}
    \item \(\phi_Z(1)=1\) so \(s=1\) is always a solution.
    \item \(\phi_Z(s)\) is a convex function and so there are at most 2 solutions.
    \item \(\phi'_Z(1)=\mu\). There is a second solution if and only if \(\mu>1\).
    \item \(\phi_Z(0)=p_0\)
\end{itemize}
For \(0\leq s\leq 1\), let \(\phi_T(s)=E[s^T]\), then:
\[\phi_T(s)=s\phi_Z(\phi_T(s))\]
\subsection*{Multiple individuals in the initial population}
So far we have only considered cases where \(X_0=1\), however we are often interested in cases where \(X_0\) follows some distribution or is equal to some value other than 1. We calculate quantities of interest by conditioning on the size of the initial generation.
\[E[X_n\mid X_0=x]=x\mu^n\]
Hence by the law of total expectation:
\[E[X_n]=E[E[X_n\mid X_0]]=E[X_0\mu^n]=\mu^nE[X_0]\]
Let \(\phi_0(s)=E[s^{X_0}]\), the pgf of the size of the initial generation. Then the probability of extinction is given by:
\[\phi_0(\gamma)=E[\gamma^{X_0}]=\sum^\infty_{j=0}\gamma^jP(X_0=j)\]
\end{document}
